...
SLOT_COUNT	= 6
BASE_CHR_ID	= 3
RACE_EISENJAEGER_M = playerSettingModule.RACE_EISENJAEGER_M
RACE_EISENJAEGER_W = playerSettingModule.RACE_EISENJAEGER_W
RACE_AETHERBRUT_M = playerSettingModule.RACE_AETHERBRUT_M
RACE_AETHERBRUT_W = playerSettingModule.RACE_AETHERBRUT_W

class CreateCharacterWindow(ui.Window):

	SLOT_ROTATION = [115.0, 165.0, 215.0, 265.0, 315.0, 5.0]
	...
	def Open(self):
		...
		try:
			...
			self.NameList.append(getChild("name_eisenjaeger"))
			self.NameList.append(getChild("name_aetherbrut"))

			self.slotButtonList = []
			for slotIndex in range(SLOT_COUNT):
				self.slotButtonList.append(getChild("slot_button_%d" % slotIndex))
		...
		for slotIndex, button in enumerate(self.slotButtonList):
			button.SetEvent(ui.__mem_func__(self.__SelectSlot), slotIndex)
		...
		self.curNameAlpha	= [0.0] * SLOT_COUNT
		self.destNameAlpha	= [0.0] * SLOT_COUNT
		...
		for slotIndex, race in enumerate(maleRaceList):
			self.__MakeCharacter(0, slotIndex, race)
		for slotIndex, race in enumerate(femaleRaceList):
			self.__MakeCharacter(1, slotIndex, race)

		self.__SelectGender(app.GetRandom(MAN, WOMAN))
		self.__SelectShape(0)
		self.__SelectSlot(app.GetRandom(0, SLOT_COUNT - 1))
